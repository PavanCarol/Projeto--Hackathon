...
class Bot {
  constructor(getAuthToken, accountId) {
    ...
    this.stepsCadastro = [
      "Qual é o seu nome?",
      "Por favor me informe seu número de telefone?",
    ...
    this.stepsAgendamento = [
      "Qual é o horário desejado para o banho do seu pet? (Responda com a data DD/MM e o horário 00:00)",
      "Antes de agendar precisamos de algumas informações importantes, podemos colocar acessório no seu pet? (Responda com :Sim ou Não)",
      "Podemos passar perfume no seu pet?, perguntamos isso pois seu pet pode ter alguma alergia. (Responda com :Sim ou Não)",
      this.getServicoOpcoes(accountId), // Mostrar opções de serviço
    ];
    this.stepsClinica = [
      "Qual é o horário desejado para a consulta? (Responda com a data DD/MM e o horário 00:00)",
  ...
  async calcularValorBanho(tipoBanho) {
    try {
      ...
      const url = `https://org4d13d757.crm2.dynamics.com/api/data/v9.2/cra6a_custos?$filter=cra6a_tipodebanho eq ${encodeURIComponent(
        tipoBanho
      )} and _cra6a_idconta_value eq ${encodeURIComponent(this.accountId)}`;
  ...
  async main(message, accountId) {
    ...
    if (
      this.agendamentoFeito &&
      (mensagemBaixa.includes("agendar") ||
        mensagemBaixa.includes("banho") ||
        mensagemBaixa.includes("consulta"))
    ) {
    ...
    if (this.verificandoCadastro) {
      ...
      if (mensagemBaixa.includes("banho")) {
        ...
        mensagemBaixa.includes("clinica") ||
        mensagemBaixa.includes("clínica")
      ) {
        this.escolhendoServico = false;
      ...
      if (this.informandoClinica) {
        ...
        if (this.currentStep < this.stepsClinica.length) {
          if (this.currentStep === 1) {
            const veterinarios = await this.listarVeterinarios(accountId);
            this.currentStep++;
  ...
  async listarVeterinarios(accountId) {
    const token = await this.getAuthToken();
    // URL da API, ajustada para filtrar pelos veterinários que pertencem ao accountId
    const url = `https://org4d13d757.crm2.dynamics.com/api/data/v9.2/cra6a_clinicas?$filter=_cra6a_idconta_value eq ${accountId}&$select=cra6a_nomeveterinario,cra6a_clinicaid`;
